...
import filterBatchQueries from '../query/filterBatchQueries';
...
class DiodeStore {
  ...
  _batchQueriesEnabled: boolean;

  _batchQueries: Array<BatchQueryDefinition>;
   ...
   * @public
   */
  useBatchQueries(batchQueries: Array<BatchQueryDefinition>): void {
  ...
  _fetchQueries(
    ...
    if (this._batchQueriesEnabled) {
      queries = filterBatchQueries(queries, this._batchQueries);
    }
    ...
    return this._networkLayer.sendQueries(queries, options).then(responses => {
      ...
      const responseMap = responses.reduce((responseMap, response, id) => {
        const query = queries[id];
        const { type, fragment } = query;
        ...
        if (type === DiodeQueryTypes.BATCH) {
          ...
          const batchQueryResponseMap = resolveBatchQuery(
            query.resolve(response, options),
            initialQueries,
            options
          );
          objectAssign(responseMap, batchQueryResponseMap);
        } else {
